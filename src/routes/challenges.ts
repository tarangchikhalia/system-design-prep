...
import db from "../db";
import { create_challenge } from "../services/challenge/challenge";
const router = Router();
...
router.post("/generate-challenge", async (req: Request, res: Response) => {
  ...
  try {
    const challenge = await create_challenge(challenge_description);
...
router.put("/challenges/:id", (req: Request, res: Response) => {
  ...
  const { challenge_name, description, difficulty, topics } =
    req.body as Partial<{
      challenge_name: string;
      description: string;
      difficulty: string;
